...
4. **移除残留弹窗外壳：**
   上述补丁可以阻断异常加载链路，但早期修复版本仍会留下一个只有标题栏的“照片同步”空壳窗口。后续继续在弹窗展示链路上打补丁，禁用该问题界面的显示入口，使连接后的“本地同步选项”自动弹窗不再出现。
...
* 连接后的“本地同步选项”自动弹窗已被完全移除，不再留下只有标题栏的空壳窗口。
* 设置页面已恢复。新版 macOS 的系统 `SearchFoundation.framework` 与 HandShaker 组件同时包含 `SFButton` 类，旧版 `Preference.nib` 可能解码到错误的系统类。维护版为 HandShaker 原按钮类注册唯一别名，并让设置 NIB 使用该别名，避免同名类冲突。
