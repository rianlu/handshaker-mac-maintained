...
- 维护版通过二进制补丁跳过该问题界面的初始化逻辑，避免继续触发异常链路
- 当前文件浏览、图片预览、视频预览等核心功能已恢复可用
- 设置页面当前仍无法正常打开，点击后会导致程序卡死
...
当前维护版的修复方式，是对 `Contents/MacOS/HandShaker` 可执行文件进行 Hopper 二进制补丁，直接跳过问题界面的初始化逻辑，再重新签名并打包为 DMG。
...
![本地同步选项弹窗修复后效果](assets/readme/本地同步选项弹窗-移除后.png)

当前维护版中，设置页面暂时无法正常打开，点击后会导致程序卡死，因此使用时请避免进入设置。

修复后的维护版中，“本地同步选项”窗口只保留标题栏，不再显示原有内容区域。这不是新的故障，而是修复结果的一部分。

原因是该旧界面正是本次卡死问题的重要触发点。维护版在修复时跳过了这段有问题的界面初始化逻辑，因此保留了窗口外壳，但不再加载原有内容。这个变化不影响文件浏览、图片预览和视频预览等核心功能。
